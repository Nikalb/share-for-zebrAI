...
if os.name == "posix":
    ...
    darknet_dir = os.path.dirname('/home/yolo/darknet/') # TODO your own path from the container
    lib = ct.CDLL(darknet_dir + "/libdarknet.so", ct.RTLD_GLOBAL)
    # lib = ct.CDLL(os.path.join(os.getcwd(), "libdarknet.so"), ct.RTLD_GLOBAL)


elif os.name == "nt":
    # cwd = os.path.dirname(__file__)
    darknet_dir = os.path.dirname('/home/yolo/darknet/') # TODO your own path from the container
    os.environ["PATH"] = os.path.pathsep.join((darknet_dir, os.environ["PATH"]))
